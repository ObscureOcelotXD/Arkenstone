...
- **The admin app does NOT use the Graph Hardhat plugin.** It only needs the **HTTP URL of a deployed subgraph** (e.g. `https://api.thegraph.com/subgraphs/name/your-org/arkenstone`). With Infisical, set that as the `VITE_SUBGRAPH_URL` secret in your environment. Without Infisical, set it in the repo root `.env`.
...
4. In Infisical (or repo root `.env` if not using Infisical) set:
   ```bash
...
4. Set `VITE_SUBGRAPH_URL` in Infisical (or repo root `.env`) as in Option A.
