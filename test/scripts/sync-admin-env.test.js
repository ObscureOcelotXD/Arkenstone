...
describe("sync-admin-env", function () {
  ...
  it("should write admin/.env with staking address, RPC, and subgraph URL", function () {
    const deployDir = path.join(tmpDir, "ignition", "deployments", "chain-31337");
    fs.mkdirSync(deployDir, { recursive: true });
    fs.writeFileSync(
      path.join(deployDir, "deployed_addresses.json"),
    ...
    const envPath = path.join(tmpDir, "admin", ".env");
  ...
  it("should update existing VITE_STAKING_ADDRESS and preserve existing VITE_SUBGRAPH_URL", function () {
    const deployDir = path.join(tmpDir, "ignition", "deployments", "chain-31337");
    fs.mkdirSync(deployDir, { recursive: true });
    fs.writeFileSync(
      path.join(deployDir, "deployed_addresses.json"),
    ...
    const result = syncAdminEnv(tmpDir);
    expect(result.updated).to.equal(true);
    const env = fs.readFileSync(path.join(adminDir, ".env"), "utf8");
